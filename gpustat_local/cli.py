...
from gpustat_local import __version__
from gpustat_local.core import GPUStatCollection
...
def print_gpustat(json=False, debug=False, **kwargs):
    ...
    if json:
        return gpu_stats.print_json(sys.stdout)
    else:
...
def main(*argv):
    ...
    parser.add_argument('-v', '--version', action='version',
                        version=('gpustat_local %s' % __version__))
    ...
    if args.interval > 0:
        ...
        del args.interval
        return print_gpustat(**vars(args))


def get_gpu_info():
    return print_gpustat(json=True)
