"""
Unit or integration tests for gpustat_local
"""
# flake8: ignore=E501
...
from gpustat_local.nvml import pynvml
...
class TestGPUStat(object):
    """A pytest class suite for gpustat_local."""
    ...
    def test_new_query_mocked(self, scenario_basic):
        ...
        gpustats = gpustat_local.new_query()
        fp = StringIO()
    ...
    def test_attributes_and_items(self, scenario_basic):
        ...
        g = gpustat_local.new_query()[1]  # includes N/A
    ...
    def test_args_commandline(self, scenario_basic):
        ...
        s = capture_output('gpustat_local', )
        assert _remove_ansi_codes_and_header_line(s) == MOCK_EXPECTED_OUTPUT_DEFAULT

        s = capture_output('gpustat_local', '--version')
        assert s.startswith('gpustat_local ')
        print(s)

        s = capture_output('gpustat_local', '--no-header')
        assert "[0]" in s.splitlines()[0]

        s = capture_output('gpustat_local', '-a')  # --show-all
        assert _remove_ansi_codes_and_header_line(s) == MOCK_EXPECTED_OUTPUT_FULL

        s = capture_output('gpustat_local', '--color')
        assert '\x0f' not in s, "Extra \\x0f found (see issue #32)"
        assert _remove_ansi_codes_and_header_line(s) == MOCK_EXPECTED_OUTPUT_DEFAULT

        s = capture_output('gpustat_local', '--no-color')
        unescaped = remove_ansi_codes(s)
    ...
    def test_args_commandline_width(self, scenario_basic):
        ...
        s = capture_output('gpustat_local', '--gpuname-width', '25')
        ...
        s = capture_output('gpustat_local', '--gpuname-width', '10')
        print(s)
        ...
        s = capture_output('gpustat_local', '--gpuname-width', '1')
        print(s)
        assert '… |' in remove_ansi_codes(s)

        print("- Should have width=0: no name displayed.")
        s = capture_output('gpustat_local', '--gpuname-width', '0')
        print(s)
        ...
        with pytest.raises(AssertionError, match="Argparse failed"):
            s = capture_output('gpustat_local', '--gpuname-width', '-1')
        with pytest.raises(AssertionError, match="Argparse failed"):
            s = capture_output('gpustat_local', '--gpuname-width', 'None')

    def test_args_commandline_showoptions(self, scenario_basic):
        """Tests gpustat_local CLI with a variety of --show-xxx options. """
        ...
        for opt in TEST_OPTS:
            ...
            print('\x1b[30m\x1b[43m',  # black_on_yellow
                  '$ gpustat_local ' + ' '.join(shlex.quote(o) for o in opt),
                  '\x1b(B\x1b[m', sep='')
            s = capture_output('gpustat_local', *opt)
        ...
        with pytest.raises(AssertionError):
            capture_output('gpustat_local', '--unrecognized-args-in-test')

    @pytest.mark.skipif(sys.platform == 'win32', reason="Do not run on Windows")
    def test_no_TERM(self, scenario_basic, monkeypatch):
        """--color should work well even when executed without TERM,
        e.g. ssh localhost gpustat_local --color"""
        monkeypatch.setenv("TERM", "")

        s = self.capture_output('gpustat_local', '--color', '--no-header').rstrip()
        print(s)
        assert remove_ansi_codes(s) == MOCK_EXPECTED_OUTPUT_DEFAULT, \
            "wrong gpustat_local output"
